...
def plot_fit(n):
    ...
    for i in range(1,n_data+1):
        if i in df_outliers_true[0].values[1:]:
            plt.plot(df_data[0].values[i-1],df_data[1].values[i-1],"ro",ms=0.8)
        else:
            plt.plot(df_data[0].values[i-1],df_data[1].values[i-1],"ko",ms=0.8)
        

    # plt.plot(outliers[0],outliers[1],'ro',mfc='none',ms=5,mew=0.5)
    # plt.plot(t,models.andreani(t,*df_sol.values[0]),lw=1,label="OVO")
    # plt.plot(t,models.andreani(t,*df_sol_ls[0].values),lw=1,label="Least Squares")
